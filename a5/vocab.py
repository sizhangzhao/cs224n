...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                ...
                listChr = [];
                for chr in word:
                    listChr.append(self.char2id[chr])
                listWord.append(listChr)
            word_ids.append(listWord)
        return(word_ids)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents = pad_sents_char(sents, self.char2id["<pad>"])
        np_sents : np.array = np.array(padded_sents)
        dim = np_sents.shape
        torch_sents = torch.from_numpy(np.array(padded_sents)).long().to(device)
        torch_sents = torch_sents.view([dim[1], dim[0], dim[2]])
        return torch_sents
