...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        unpacked_X = self.model_embeddings.source(source_padded)
        X = nn.utils.rnn.pack_padded_sequence(unpacked_X, source_lengths)
        ...
        init_decoder_hidden = self.h_projection(last_hidden_reformat)
        last_cell_reformat = torch.cat([last_cell[0, :], last_cell[1, :]], 1)
        init_decoder_cell = self.c_projection(last_cell_reformat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        tgt_embedding = self.model_embeddings.target(target_padded)
        for each_tgt_embedding in torch.split(tgt_embedding, 1):
            each_tgt_embedding = torch.squeeze(each_tgt_embedding)
            y_hat = torch.cat([each_tgt_embedding, o_prev], 1)
            dec_state, co, e_t = self.step(y_hat, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(co)
            o_prev = co
